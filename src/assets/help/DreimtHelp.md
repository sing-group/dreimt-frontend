...
Read carefully this manual and its examples to use DREIMT.
## Index
...
9. [FAQS](help#faqs)
    ...
    3. [Tau interpretation](help#f3)
    4. [DREIMT terminology](help#f4)
...
DSS scores ranges from **0 to 1**, scores close to 1 indicate that the transcriptomic effect of a given drug is found to be akin all tested cancer cell lines in LINCS database, on the contrary, scores close to 0 indicate the drug effect has been observed to be more cancer cell line specific. Not all drugs have DSS score available.

If multiple drugs are associated (tau score >|90|) with an immune expression signature, DSS can be used to prioritize drugs with higher DSS score.
...
In this example a [query](http://dreimt.sing-group.org/database?cellType1Effect=INHIBIT&cellType1Treatment=Overexpression%20%5BTOX%5D&interactionType=SIGNATURE&minTau=90&cellType1=T%20cell&cellSubType1=T%20CD8%2B) to search for drugs with the potential to inhibit the effect of **TOX overexpression** (exhaustion marker) **in T CD8+ cells** will be performed. For this example only associations made with the complete immune signature (upregulated and downregulated genes) will be of interest.
...
<p align="center">
<img src="assets/help/assets/help/Example_1_signature_detailed_1.png" alt="Example1 detailed" style="max-width: 30%"></img>
</p>
...
- **Drug prioritization plot (1)**: plot representing all DREIMTdb drugs associations (|tau| > 80) with the immune signature. The x-axis represents the tau score, y-axis the -log10(FDR).
...
In this example we will perform a query to search for approved drugs with the potential to boost human macrophage cell polarization towards a macrophage M1 phenotype instead of to macrophage M2 phenotype. Initially we will need to filter immune signatures generated from the comparison of macrophages M1 and M2, then apply the rest of filters to narrow our search.
...
Both queries yield the same results, seven drugs candidate to modulate macrophage M1/M2 polarization balance. In this case filtered associations have been made using the down regulated genesets from the classical M1 macrophages vs alternative M2 macrophages signature.
...
Drug prioritization tool performs drug association prioritizations from user-provided immune gene expression signatures against 4,690 DREIMTdb drug profiles. Drug prioritization tools uses genes that are present in both, user-provided signature and DRETIMTdb profiles. Genes not present in DREIMTdb profiles will not be used for drug association prioritization.
...
- **Case cell description**: (optional) brief description of the case immune cell used to generate the signature. It is highly recommended to add a short description, this will be used to create a brief sentence to facilitate the results interpretation.

- **Reference cell description**: (optional) brief description of the reference immune cell used to generate the signature. It is highly recommended to add a short description, this will be used to create a brief sentence to facilitate the results interpretation.
...
- **Query genes**: user-provided immune signature, can be introduced as a list of genes pasted to the text boxes or as a csv/tsv file. For a complete description of input formats see [Inputs genes](help#inputs).
...
In this example the input is a signature derived from the comparison of M1 macrophages vs M2 macrophages, which corresponds to one of the [preloaded examples](http://dreimt.org/query/drug-prioritization/results/signature/a8391092-ca25-4bc2-bcb3-141c69877a1d) (Coates et al 2008). For demonstration purposes the cells are labeled as "cells with M1 markers" and "cells with M2 marker".

<p align="center">
<img src="assets/help/assets/help/EjemploPrioritization.png" alt="Example prioritization" style="max-width: 50%"></img>
</p>

After launching the query, a message will indicate the analysis is being performed, in a few minutes the results will appear in the screen. The window can be closed while the analysis is running, the results will be stored in the query [history panel](help#queries-history) with the name given in the query title.
...
**This page displays three summary plots and a table.**
- **Query parameters (1)**: panel with summarized information of the analysis:
...
- **Drug prioritization plot (2)**: plot representing all drugs associated (tau > |75|) with the user-provided immune signature. The x-axis represents the tau score, y-axis the -log10(FDR).
...
Notice how a proper case/reference cell description facilitates drug association results interpretation by creating a meaningful summary sentence.

The user-provided signature contains 81 upregulated and 78 downregulated genes, of those, 50 upregulated and 45 downregulated genes matched DREIMTdb drug profiles genes and were used to generate drug associations with the signature. Genes used for the drug association can be downloaded in plain text format by pressing the download query genes button. Result association table can be downloaded as csv format table.

A total of 332 drugs are associated (|tau| > 75) with the user-provided signature, an initial search to retrieve topmost interesting drugs hypotheses should be centered in drugs falling in the plot areas (|tau| > 90) of best candidates and good candidates.
...
After filtering associations by |tau| > 90, 49 potential drug hypotheses are candidate for further investigation.
...
Signature comparison tool performs geneset-pairwise Fisher's exact test and jaccard index analyses to test the existence of significant gene overlap between the  user-provided gene expression signature and DREIMTdb signatures. From this user can explore drug association for topmost similar DREIMTdb signatures.
...
In this example the user-provided signature will be compared to all DREIMTdb signatures. For demonstration purposes the user-provided signature is a DREIMTdb signature derived from comparing macrophages M1 vs M2, which correspond to one of the preloaded [examples](http://dreimt.sing-group.org/query/signatures-comparison/results/4ef3f98c-bee4-4cd5-a8a5-babd01f86a9e) (Coates et al 2008).

<p align="center">
<img src="assets/help/assets/help/Jaccard_panel_query1_ppt.png" alt="Panel query1" style="max-width: 50%"></img>
</p>

After launching the query, a message will indicate the analysis is being perform, in a few minutes the results will appear in the screen. The window can be closed while the analysis is running, the results will be stored in the [queries history](help#queries-history) panel with the name given in the query title.
...
- **Enrichment Cell type plot (2)**: bar-chart results for cell type enrichment analysis. This plot indicates whether the user-provided signature significantly overlaps with DREIMTdb signatures derived from a specific cell type. This analysis is based in cell types filtered in the signature comparison table (5), by default only DREIMTdb signatures with a significant gene overlap (FDR < 0.05) are accounted. This threshold can be modified using a different threshold for p-value, FDR or jaccard index. Bars are p-value ordered, length represents the significance for the specific cell type and colored for p-value < 0.05. By default all cell types are tested unless a signature filtering is specified in the input (Filters my query option). A maximum of 10 bars are shown.

- **Enrichment Cell subtype plot (3)**: bar-chart results for cell subtype enrichment analysis. This plot indicates whether the user-provided signature significantly overlaps with DREIMTdb signatures derived from a specific cell subtype. This analysis is based in cell subtypes filtered in the signature comparison table (5), by default only DREIMTdb signatures with a significant gene overlap (FDR < 0.05) are accounted. This threshold can be modified using a different threshold for p-value, FDR or jaccard index. Bars are p-value ordered, length represents the significance for the specific cell type and colored for p-value < 0.05. By default all cell types are tested unless a signature filtering is specified in the input (Filters my query option). A maximum of 10 bars are shown.

- **Enrichment Cell type raw data (4)**: raw data used to generate the enrichment cell type plot, table can be downloaded in csv format. A maximum of 10 bars are shown.

- **Enrichment Cell subtype raw data (5)**: raw data used to generate the enrichment cell subtype plot, table an be downloaded in csv format.
...
- **Signature comparison table (6)**: table with summarized pairwise user-provided-DREIMTdb geneset comparisons, by default only genesets with significant overlaps are shown (FDR < 0.05).
 ...
 - **Jaccard**: Jaccard index value from the comparison of the user-provided immune geneset against DREIMTdb immune geneset. Includes a link to download a file with common genes of the user-provided and DREIMTdb geneset.

<p align="center">
<img src="assets/help/assets/help/ComparisonEx1.png" alt="Comparison 1" style="max-width: 50%"></img>
</p>

The bar plots indicate that the user-provided signature significantly (p-value < 0.05) overlaps with DREIMTdb signatures derived from macrophage and monocyte (cell types) and macrophage M1, macrophage, monocyte and macrophage M2 (cell subtypes). By default only signatures with a significant (Fisher's exact test FDR < 0.05) gene overlap between user-provided geneset and DREIMTdb geneset are considered for this analysis. This can be modified by applying other FDR, p-value or jaccard threshold values, this will update bar plot results.
...
In this example the user-provided signature corresponds to a signature included in DREIMTdb, both the upregulated and downregulated genesets have a perfect match (jaccard index = 1) with its respective genesets.

For signatures of interest (generally those most similar to user-provided signature) information interest can be retrieved using the pop-up panel in the Target signature information column:
...
#### 9.3 Tau interpretation<a name="f3"></a>
...
#### 9.4 DREIMT terminology<a name="f4"></a>
